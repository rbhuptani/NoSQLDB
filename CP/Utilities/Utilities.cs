...
namespace Project4Starter
{
  static public class Utilities
  {
    ...
    static public string processCommandLineForLog(string[] args, string log)
    {
      for (int i = 0; i < args.Length; ++i)
      {
        if ((args.Length > i + 1) && (args[i].ToLower() == "/log"))
        {
          log = args[i + 1];
        }
      }
    ...
    static public string getDBType(string[] args,string type)
    {
      for (int i = 0; i < args.Length; ++i)
      {
        if ((args.Length > i + 1) && (args[i].ToLower() == type.ToLower()))
        {
          type = args[i + 1];
        }
      }
    ...
    static public int getmsgsCount(string[] args,string type)
    {
      int ans=10;
      for (int i = 0; i < args.Length; ++i)
      {
